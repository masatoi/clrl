...
(set 'model (rbf-model-initializer
	     ;; mean-list
	     (mapcar #'list->vector (let ((dip (direct-product (seq -1.0d0 1.0d0 :by 0.2d0)
							       (seq -1.0d0 1.0d0 :by 0.2d0))))
...
(nlet itr ((x.dat x_in.dat1.schaal)
  ...
  (if (null x.dat)
	...
	(let ((w-new (LMS-algorithm (car x.dat) (car t.dat) w model
				    :eta eta :regularization-lambda 0d0)))
	  ;(print (m* (m-t w-new) w))
